...
const JOURNEY_STAGES = [
  {
    label: 'Weekly Revenue Reporting',
    ...
    <div className="flex flex-col h-full bg-slate-900 border-r border-slate-800">
      ...
      <div className="px-5 py-5 border-b border-slate-800 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="text-white text-sm font-semibold leading-tight">Product Intelligence</div>
            <div className="text-slate-500 text-xs">by Mindvalley</div>
          </div>
        </div>
        {onClose && (
          <button
        ...
        <div>
          ...
          <ul className="space-y-0.5">
            {JOURNEY_STAGES.map((stage) => {
              const active = !stage.locked && isActive(stage.path, stage.exact);
              if (stage.locked) {
                return (
                  <li key={stage.path}>
                    <Link
                      href="/locked"
                      onClick={() => onClose?.()}
                      className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-slate-600 hover:text-slate-500 hover:bg-slate-800/30 transition-all"
                    >
                      <span className="text-slate-700">{stage.icon}</span>
        ...
        <div>
          ...
          <ul className="space-y-0.5">
            {INTEL_LINKS.map((link) => {
              const active = !link.locked && isActive(link.path);
              if (link.locked) {
                return (
                  <li key={link.path}>
                    <Link
    ...
    <>
      ...
      <div className="sm:hidden fixed top-0 left-0 right-0 z-30 flex items-center gap-3 bg-slate-900 border-b border-slate-800 px-4 py-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-white text-sm font-semibold">Product Intelligence</span>
        </div>
      </div>
