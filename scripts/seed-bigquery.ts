...
function seedFromJson(db: Database.Database): number {
  ...
  if (fs.existsSync(JSON_PATH)) {
    console.log('[seed] Falling back to', JSON_PATH);
    lines = fs.readFileSync(JSON_PATH, 'utf-8').split('\n');
  } else if (fs.existsSync(JSON_GZ_PATH)) {
    console.log('[seed] Falling back to', JSON_GZ_PATH);
    lines = zlib.gunzipSync(fs.readFileSync(JSON_GZ_PATH)).toString('utf-8').split('\n');
  } else {
    console.log('[seed] No local JSON file found — skipping');
    return 0;
  }
  ...
  insertAll();
  console.log(`[seed] Inserted ${inserted} rows from JSON`);
  return inserted;
}
...
main().catch(err => {
  console.error('[seed] BigQuery error — falling back to JSON:', err);
  const startedAt = new Date().toISOString();
  try {
    if (!fs.existsSync(DB_DIR)) fs.mkdirSync(DB_DIR, { recursive: true });
    const db = new Database(DB_PATH);
    ...
    console.error('[seed] JSON fallback also failed:', fallbackErr);
    try { fs.writeFileSync(STATUS_PATH, JSON.stringify({ status: 'skipped', startedAt, completedAt: new Date().toISOString() })); } catch { /* ignore */ }
  }
  process.exit(0); // always exit 0 so the server starts
});
